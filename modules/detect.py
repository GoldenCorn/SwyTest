...
def findtemplate(image, template, threshold=0.75, outline=False):
    ...
    #print(f"image type: {image.dtype}, image shape: {image.shape}, image dim: {image.ndim}")
    #print(f"template type: {template.dtype}, template shape: {template.shape}, template dim: {template.ndim}")
